  ...
  return <>
    ...
    <section className="py-24 md:py-32">
      <div className="site-container grid gap-12 lg:grid-cols-12 lg:items-center">
        <div className="group relative lg:col-span-7"><div className="absolute -left-5 -top-5 h-full w-full border border-primary/20"/><img src={teamImage.url} alt="Interdisziplinäre Projektbesprechung" width={1600} height={1072} className="image-lift relative aspect-[16/10] w-full object-cover" /></div>
        <div className="lg:col-span-4 lg:col-start-9"><p className="eyebrow text-primary">Denaro Consulting GmbH</p><h2 className="mt-5 font-display text-4xl font-semibold leading-tight">Beratung mit Blick auf das <span className="editorial text-primary">Ganze.</span></h2><p className="mt-7 leading-8 text-muted-foreground">Unternehmerische Fragen lassen sich selten isoliert betrachten. Prozesse beeinflussen Systeme, Systeme prägen Entscheidungen und Projekte brauchen eine klare Struktur. Genau hier setzen wir an: mit einem interdisziplinären Blick und einer Zusammenarbeit auf Augenhöhe.</p></div>
      </div>
    </section>
    <section className="bg-secondary py-24 md:py-32"><div className="site-container grid gap-12 lg:grid-cols-12 lg:items-center"><div className="lg:col-span-4"><img src={officeImage.url} alt="Wirtschaftsstandort Düsseldorf" width={1600} height={1008} loading="lazy" className="aspect-[3/4] w-full object-cover" /></div><div className="lg:col-span-6 lg:col-start-7"><p className="eyebrow text-primary">Geschäftsführung</p><h2 className="mt-5 font-display text-4xl font-semibold">Alexander Valentino Denaro</h2><blockquote className="mt-8 border-l-2 border-primary pl-7 font-display text-2xl font-medium leading-relaxed text-foreground md:text-3xl">„Gute Beratung schafft Orientierung und macht aus einer komplexen Aufgabe einen <span className="editorial text-primary">klaren nächsten Schritt.</span>“</blockquote><p className="mt-8 leading-8 text-muted-foreground">Vom Standort Düsseldorf aus begleiten wir Unternehmen und Projekte mit persönlicher Verantwortung, direkter Kommunikation und einem konsequenten Blick auf umsetzbare Ergebnisse.</p></div></div></section>
    <ContactBand />
  </>;
}
